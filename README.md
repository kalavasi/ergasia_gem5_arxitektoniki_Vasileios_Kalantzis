...
**A.** Το [πρόγραμμα](files/number_folder). Tα [αποτελέσματα για minorCPU](files/number_result/Minor_CPU_number) και [αποτελέσματα για TImingSimpleCPU](files/number_result/Timing_Simple_CPU_number)
...
2. [Οι τιμές του gem5. Το αρχείο χωρίς κατάληξη είναι το default, τα αρχεία με κατάληξη 1 και 4 GHz είναι για αλλάγες στο cpu clock και το DDR είναι με την αλλάγή στην μνήμη](files/spec_results.zip)
...
Το μέγεθος της cache επηρεάζει γραμμικά το κόστος υλοποίησης, καθώς η αύξησή του οδηγεί σε αναλογική αύξηση του αριθμού των SRAM cells. Το associativity, από την άλλη πλευρά, δεν επηρεάζει το κόστος με τον ίδιο τρόπο, αφού η αύξησή του συνεπάγεται με περισσότερους συγκριτές και πιο σύνθετη λογική, αυξάνοντας την πολυπλοκότητα του κυκλώματος. Για τον λόγο αυτό το associativity επηρεάζει το latency πιο ήπια και μπορεί να προσεγγιστεί λογαριθμικά. Το cache line size δεν επηρεάζει άμεσα το hit latency της cache, αλλά επηρεάζει το miss penalty, καθώς σε περίπτωση αστοχίας πρέπει να μεταφερθεί ολόκληρο το cache line από το επόμενο επίπεδο μνήμης, αυξάνοντας τον χρόνο πρόσβασης. Επιπλέον η L1 cache θεωρείται σημαντικά ακριβότερη σε σχέση με την L2, καθώς απαιτεί πολύ χαμηλό latency.

Με βάση τα παραπάνω, μια ενδεικτική συνάρτηση κόστους που αποτυπώνει ποιοτικά τα παραπάνω trade-offs μπορεί να γραφτεί ως:

Total_Cost =
  w1 · [ L1 · (α·S_L1 + β·A_L1) + L2 · (α·S_L2 + β·A_L2) ] + w2 · [ (δ·S_L1 + ε·log₂(A_L1)) + (δ·S_L2 + ε·log₂(A_L2)) ] + w3 · [ θ·CL_L1 · Miss_Rate_L1 + θ·CL_L2 · Miss_Rate_L2 ]

H πρώτη παρένθεση με τον όρο w1 είναι το κόστος μεγέθους, η δεύτερη είναι ο χρόνος που χρειάζεται για να γίνει hit και η τρίτη είναι για την καθυστέρηση της μνήμης για miss

όπου S_L1 S_L2 το μέγεθος των cache, A_L1 A_L2 το associativity, CL_1 CL_2 το cache line size που είναι ίσα. Η συνάρτηση αυτή δεν στοχεύει σε ακριβή ποσοτική αποτίμηση του κόστους, αλλά στην ποιοτική κατανόηση των σχεδιαστικών επιλογών και των επιπτώσεών τους στην απόδοση και την πολυπλοκότητα, όπως ακριβώς γίνεται και στον σχεδιασμό σύγχρονων επεξεργαστών.

Με βάση τα αποτελέσματα του Βήματος 2 και τη συνάρτηση κόστους που κατασκευάστηκε, παρατηρούμε ότι η αρχιτεκτονική με το μικρότερο CPI δεν είναι πάντα η βέλτιστη επιλογή όταν συνυπολογιστεί το κόστος υλοποίησης. Για τα περισσότερα benchmarks, όπως τα specbzip και specmcf, η μετάβαση από το test_9 στο test_10 προσφέρει σχετικά μικρή βελτίωση στο CPI, ενώ το κόστος αυξάνεται σημαντικά λόγω μεγαλύτερων cache, υψηλότερου associativity και πολύ μεγάλου cache line size. Αυτό υποδεικνύει ότι το test_9 προσφέρει καλύτερη ισορροπία μεταξύ απόδοσης και κόστους.

Αντίθετα, σε benchmarks που χρησιμοποιούν πολύ την μνήμη όπως τα speclibm και specsjeng, παρατηρείται δραματική μείωση του CPI μόνο στις πιο επιθετικές cache αρχιτεκτονικές, γεγονός που δικαιολογεί το αυξημένο κόστος του test_10. Επομένως, η βέλτιστη αρχιτεκτονική εξαρτάται από το είδος του workload: για γενική χρήση προτείνεται μια ενδιάμεση λύση όπως το test_9, ενώ για εφαρμογές με έντονη χρήση μνήμης η επιλογή του test_10 μεγιστοποιεί την απόδοση παρά το αυξημένο κόστος. Οι συμβιβασμοί αυτοί αντικατοπτρίζουν τις πραγματικές αποφάσεις που λαμβάνονται στη σχεδίαση σύγχρονων επεξεργαστών.
