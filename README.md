...
Το μέγεθος της cache επηρεάζει γραμμικά το κόστος υλοποίησης, καθώς η αύξησή του οδηγεί σε αναλογική αύξηση του αριθμού των SRAM cells. Το associativity, από την άλλη πλευρά, επηρεάζει το κόστος, αφού η αύξησή του συνεπάγεται με περισσότερους συγκριτές και πιο σύνθετη λογική, αυξάνοντας την πολυπλοκότητα του κυκλώματος. Το cache line size δεν επηρεάζει άμεσα το hit latency της cache, αλλά επηρεάζει το miss penalty, καθώς σε περίπτωση αστοχίας πρέπει να μεταφερθεί ολόκληρο το cache line από το επόμενο επίπεδο μνήμης, αυξάνοντας τον χρόνο πρόσβασης. Επιπλέον η L1 cache θεωρείται σημαντικά ακριβότερη σε σχέση με την L2, καθώς απαιτεί πολύ χαμηλό latency.

Με βάση τα παραπάνω, μια ενδεικτική συνάρτηση κόστους που αποτυπώνει ποιοτικά τα παραπάνω trade-offs μπορεί να γραφτεί ως:

Total_Cost =
  w1 · [ L1 · (α·S_L1 + β·A_L1) + L2 · (α·S_L2 + β·A_L2) ] + w2 · [ (δ·S_L1 + ε·(A_L1)) + (δ·S_L2 + ε·(A_L2)) ] + w3 · [ θ·CL_L1 · Miss_Rate_L1 + θ·CL_L2 · Miss_Rate_L2 ]
